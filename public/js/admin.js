...
function adminSwitchTab(tab) {
  ...
  ['dashboard', 'pending', 'schedule', 'classrooms', 'users', 'logs'].forEach(t => {
...
function handleBatchFile(input) {
  ...
  selectedFile = file;
  document.getElementById('selectedFileName').textContent = `已选择：${file.name} (${(file.size / 1024).toFixed(1)}KB)`;
  document.getElementById('selectedFileName').style.display = 'block';
  document.getElementById('batchUploadBtn').style.display = 'inline-flex';
}

async function uploadBatchFile() {
  if (!selectedFile || !adminUser) return;
  ...
  try {
    const res = await fetch('/api/users/batch-register', {
      method: 'POST',
      body: formData
    });
    ...
    if (!data.success) {
      ...
      document.getElementById('batchResultArea').innerHTML = `
        <div style="padding:16px;background:#FFF0F0;border-radius:6px;color:#CF1322">
          <strong>导入失败：</strong>${data.message}
        </div>
      `;
      return;
    }
    ...
    let resultHtml = `
      <div class="batch-result-summary">
        <div class="batch-result-card" style="background:#F0F5FF">
          <div class="num" style="color:var(--primary)">${result.total}</div>
          <div class="lbl">总记录数</div>
        </div>
        <div class="batch-result-card" style="background:#E8F8E0">
          <div class="num" style="color:#389E0D">${result.success}</div>
          <div class="lbl">成功导入</div>
        </div>
        <div class="batch-result-card" style="background:${result.failed > 0 ? '#FFF0F0' : '#F5F5F5'}">
          <div class="num" style="color:${result.failed > 0 ? '#CF1322' : 'var(--text-muted)'}">${result.failed}</div>
          <div class="lbl">导入失败</div>
        </div>
      </div>
    `;
    ...
    if (result.errors && result.errors.length > 0) {
      resultHtml += `
        <div style="font-weight:600;font-size:14px;margin-bottom:8px;color:#CF1322">失败详情：</div>
        <div class="batch-error-list">
          ${result.errors.map(e => `
            <div class="batch-error-item">
              <span style="color:var(--text-muted);min-width:60px">第${e.line}行</span>
              <span style="font-weight:500">${e.name}</span>
              <span style="color:#CF1322">${e.error}</span>
            </div>
          `).join('')}
        </div>
      `;
      ...
      resultHtml += `
        <div style="padding:16px;background:#E8F8E0;border-radius:6px;text-align:center;color:#389E0D;font-weight:500">
          全部导入成功！新注册用户的账号与密码均为学号/工号
        </div>
      `;
    }

    document.getElementById('batchResultArea').innerHTML = resultHtml;
  } catch (err) {
    document.getElementById('uploadProgress').style.display = 'none';
...
function downloadCSVTemplate() {
  const template = '姓名,学号,手机号,部门/班级\n张三,20230001,13800001111,新闻学1班\n李四,20230002,13800002222,传播学2班';
